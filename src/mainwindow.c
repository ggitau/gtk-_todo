...
on_task_added(GObject* obj, gpointer data){
    ...
    GQuark quark = g_quark_from_string (task->id);
    g_datalist_id_set_data_full (&data_list, quark, task, destroy_task);
    /*g_datalist_foreach(&data_list,print_all_tasks,NULL);*/

}

static gboolean on_main_window_delete(GtkWidget *widget ,GdkEvent *event
        , gpointer data){
    g_print("Storing tasks\n");
    store_tasks();
    return FALSE;
}
static void connect_signals(){
    assert(mainwindow!=NULL);
    g_signal_connect(GTK_WIDGET(mainwindow),"delete-event",G_CALLBACK(on_main_window_delete),NULL);
    g_signal_connect(task_window,"todotask-added",G_CALLBACK(on_task_added),NULL);
}
